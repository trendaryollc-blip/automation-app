...
import { testCJDropshipping, placeCJOrder, getCJInventory } from '../../src/services/cjdropshipping';
...
describe('cjdropshipping', () => {
  ...
  describe('placeCJOrder', () => {
    it('throws when credentials not configured', async () => {
      ...
      await expect(placeCJOrder({
        productId: '123', quantity: 1,
        shippingInfo: { firstName: 'A', lastName: 'B', email: 'a@b.com', phone: '123', address: 'Addr', city: 'City', state: 'ST', zip: '123', country: 'US' },
  ...
  describe('getCJInventory', () => {
    it('throws when credentials not configured', async () => {
      ...
      await expect(getCJInventory('123')).rejects.toThrow('CJ Dropshipping API credentials not configured');
