...
describe('Orders Route', () => {
  ...
  describe('GET /orders', () => {
    ...
    it('returns seeded orders', async () => {
      seedTable('orders', [{ orderNumber: 'ORD-001', customerName: 'Alice', status: 'pending', sellPrice: '100', quantity: 1 }]);
      const res = await request(createApp()).get('/orders');
  ...
  describe('POST /orders', () => {
    it('creates an order with valid data', async () => {
      const res = await request(createApp()).post('/orders').send({
        productName: 'Widget', customerName: 'Bob', sellPrice: 50, costPrice: 20, quantity: 2,
      });
  ...
  describe('POST /orders/bulk-update', () => {
    it('updates multiple orders status', async () => {
      ...
      const res = await request(createApp()).post('/orders/bulk-update').send({ orderIds: [1, 2], status: 'placed' });
  ...
  describe('POST /orders/import', () => {
    it('imports orders from rows', async () => {
      const res = await request(createApp()).post('/orders/import').send({ rows: [{ productName: 'Imported' }] });
  ...
  describe('GET /orders/:id', () => {
    it('returns a specific order', async () => {
      const [o] = seedTable('orders', [{ orderNumber: 'O3', customerName: 'Charlie', status: 'pending' }]);
  ...
  describe('PATCH /orders/:id', () => {
    it('updates an order', async () => {
      const [o] = seedTable('orders', [{ orderNumber: 'O4', customerName: 'Dave', status: 'pending' }]);
      const res = await request(createApp()).patch(`/orders/${o.id}`).send({ status: 'shipped', trackingNumber: 'TRACK123' });
      expect(res.status).toBe(200);
  ...
  describe('DELETE /orders/:id', () => {
    it('deletes an order', async () => {
      const [o] = seedTable('orders', [{ orderNumber: 'O5', customerName: 'Eve', status: 'pending' }]);
