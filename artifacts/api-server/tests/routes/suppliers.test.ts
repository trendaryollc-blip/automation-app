...
describe('Suppliers', () => {
  ...
  it('GET /suppliers filters by country', async () => {
    seedTable('suppliers', [{ name: 'A', country: 'China' }, { name: 'B', country: 'USA' }]);
    const res = await request(express().use(suppliersRouter)).get('/suppliers?country=China');
  ...
  it('GET /suppliers/:id returns 404', async () => {
    const res = await request(express().use(suppliersRouter)).get('/suppliers/999');
