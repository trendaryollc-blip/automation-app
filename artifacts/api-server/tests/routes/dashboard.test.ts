...
describe('Dashboard', () => {
  ...
  it('GET /dashboard/stats returns zeros', async () => {
    const res = await request(express().use(dashboardRouter)).get('/dashboard/stats');
  ...
  it('GET /dashboard/analytics returns weekly data', async () => {
    const res = await request(express().use(dashboardRouter)).get('/dashboard/analytics');
    expect(res.body.data).toHaveLength(7);
  });

  it('GET /dashboard/analytics?period=monthly returns 12 months', async () => {
    const res = await request(express().use(dashboardRouter)).get('/dashboard/analytics?period=monthly');
