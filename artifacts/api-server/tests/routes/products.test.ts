...
describe('Products', () => {
  ...
  it('GET /products returns seeded', async () => {
    seedTable('products', [{ name: 'Widget', status: 'listed', costPrice: '5', sellPrice: '15' }]);
  ...
  it('GET /products/stock-alerts', async () => {
    seedTable('products', [{ name: 'Low', stockQuantity: 2, stockThreshold: 10, status: 'listed' }]);
    const res = await request(express().use(productsRouter)).get('/products/stock-alerts');
    expect(res.body[0].name).toBe('Low');
  });

  it('GET /products/:id returns 404', async () => {
    const res = await request(express().use(productsRouter)).get('/products/999');
