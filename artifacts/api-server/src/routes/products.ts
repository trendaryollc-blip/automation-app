...
router.post("/products", async (req, res): Promise<void> => {
  ...
  res.status(201).json({
    ...
    margin:
      product.costPrice != null && product.sellPrice != null && Number(product.sellPrice) > 0
        ? Math.round(((Number(product.sellPrice) - Number(product.costPrice)) / Number(product.sellPrice)) * 100)
        : null,
    ...
    .map((p) => {
      ...
      const margin =
        cost != null && sell != null && sell > 0
          ...
          : null;
      return { ...p, costPrice: cost, sellPrice: sell, margin, createdAt: p.createdAt.toISOString(), updatedAt: p.updatedAt.toISOString() };
    })
...
router.post("/products/import", async (req, res): Promise<void> => {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      const costPrice = row.costPrice != null && row.costPrice !== "" ? String(Number(row.costPrice)) : undefined;
      const sellPrice = row.sellPrice != null && row.sellPrice !== "" ? String(Number(row.sellPrice)) : undefined;
      const [product] = await db
        .insert(productsTable)
        .values({
          ...
          stockThreshold: row.stockThreshold != null ? Number(row.stockThreshold) : undefined,
...
router.post("/products/:id/generate-description", async (req, res): Promise<void> => {
  ...
  const sellPrice = product.sellPrice != null ? `$${Number(product.sellPrice).toFixed(2)}` : "";

  let aiDescription: string;
  try {
    const { generateDescription } = await import("../services/ai.js");
    aiDescription = await generateDescription(name, category, niche, sellPrice || undefined);
  ...
  res.json({
    ...
    margin:
      updated.costPrice != null && updated.sellPrice != null && Number(updated.sellPrice) > 0
        ? Math.round(((Number(updated.sellPrice) - Number(updated.costPrice)) / Number(updated.sellPrice)) * 100)
        : null,
