...
router.get("/promotions", async (req, res) => {
  const all = await db.select().from(promotionsTable).orderBy(promotionsTable.createdAt);
...
router.patch("/promotions/:id", async (req, res) => {
  ...
  const [updated] = await db.update(promotionsTable).set({ ...req.body, updatedAt: new Date() }).where(eq(promotionsTable.id, id)).returning();
