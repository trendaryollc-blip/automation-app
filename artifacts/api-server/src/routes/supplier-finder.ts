...
const SUGGESTED_SUPPLIERS: Record<string, { name: string; country: string; website: string; email: string }[]> = {
  tech: [
    { name: "ShenZhen TechLink Co.", country: "China", website: "https://techlink.cn", email: "sourcing@techlink.cn" },
    { name: "HK Elite Electronics", country: "Hong Kong", website: "https://hkelite.com", email: "b2b@hkelite.com" },
    { name: "Taiwan Components Direct", country: "Taiwan", website: "https://twcomponents.com", email: "sales@twcomponents.com" },
  ],
  health: [
    { name: "WellSource Global", country: "China", website: "https://wellsource.cn", email: "bulk@wellsource.cn" },
    { name: "FitGoods EU", country: "Germany", website: "https://fitgoods.eu", email: "orders@fitgoods.eu" },
    { name: "ActiveLife Wholesale", country: "United States", website: "https://activelife.us", email: "wholesale@activelife.us" },
  ],
  home: [
    { name: "HomeStyle Imports", country: "China", website: "https://homestyleimports.com", email: "trade@homestyleimports.com" },
    { name: "EuroDecor BV", country: "Netherlands", website: "https://eurodecor.nl", email: "info@eurodecor.nl" },
    { name: "CasaVida Wholesale", country: "Spain", website: "https://casavida.es", email: "pedidos@casavida.es" },
  ],
  kitchen: [
    { name: "KitchenPro Exports", country: "China", website: "https://kitchenpro.cn", email: "export@kitchenpro.cn" },
    { name: "Nordic Kitchen Supply", country: "Denmark", website: "https://nordickitchen.dk", email: "bulk@nordickitchen.dk" },
    { name: "FreshGoods USA", country: "United States", website: "https://freshgoods.us", email: "wholesale@freshgoods.us" },
  ],
  default: [
    { name: "GlobalSource Direct", country: "China", website: "https://globalsource.cn", email: "inquiry@globalsource.cn" },
    { name: "EuroDrop Logistics", country: "Poland", website: "https://eurodrop.pl", email: "drop@eurodrop.pl" },
    { name: "AmeriSource Wholesale", country: "United States", website: "https://amerisource.us", email: "b2b@amerisource.us" },
  ],
};
...
function detectCategory(name: string): string {
  ...
  if (/phone|laptop|charge|cable|usb|gadget|electronic|wifi|watch|earbud|speaker|tech/.test(n)) return "tech";
  if (/health|fitness|gym|yoga|posture|muscle|vitamin|workout|exercise|belt|corrector/.test(n)) return "health";
...
router.post("/suppliers/find", async (req, res): Promise<void> => {
  ...
  const existingSuppliers = await db.select().from(suppliersTable).orderBy(desc(suppliersTable.rating));
  ...
  for (const s of existingSuppliers) {
    ...
    const countryBonus = preferredCountry && s.country === preferredCountry ? 15 : 0;
    const shippingBonus = s.shippingDays != null && s.shippingDays <= 7 ? 10 : s.shippingDays != null && s.shippingDays <= 14 ? 5 : 0;
  ...
  for (const suggested of shuffled) {
    if (preferredCountry && suggested.country !== preferredCountry && rand() > 0.3) continue;
    const baseCost = targetCostPrice
  ...
  const tips = (SOURCING_TIPS[category] ?? SOURCING_TIPS.default).slice(0, 3 + Math.floor(rand() * 1));

  const [record] = await db.insert(supplierFinderTable).values({
    productName: productName.trim(),
    targetCostPrice: targetCostPrice != null ? String(targetCostPrice) : undefined,
  ...
  res.json({
    ...record,
    targetCostPrice: record.targetCostPrice != null ? Number(record.targetCostPrice) : null,
...
router.get("/suppliers/find/history", async (_req, res): Promise<void> => {
  const rows = await db.select().from(supplierFinderTable).orderBy(desc(supplierFinderTable.createdAt)).limit(50);
  res.json(rows.map((r) => ({
    ...r,
    targetCostPrice: r.targetCostPrice != null ? Number(r.targetCostPrice) : null,
...
router.delete("/suppliers/find/history/:id", async (req, res): Promise<void> => {
  const params = DeleteSupplierFinderResultParams.safeParse(req.params);
  if (!params.success) {
    res.status(400).json({ error: params.error.message });
    return;
  }
  const [row] = await db.delete(supplierFinderTable).where(eq(supplierFinderTable.id, params.data.id)).returning();
  if (!row) {
