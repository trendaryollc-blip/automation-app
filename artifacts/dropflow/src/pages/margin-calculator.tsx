...
export default function MarginCalculator() {
  ...
  const results = useMemo(() => {
    ...
    const actualMargin =
      totalCost > 0 ? ((sellPrice - totalCost) / sellPrice) * 100 : 0;
    const markup =
      totalCost > 0 ? ((sellPrice - totalCost) / totalCost) * 100 : 0;
    ...
    const breakEvenRows: BreakEvenRow[] = adSpendLevels.map((adSpend) => {
      ...
      const breakEvenUnits =
        fixedCosts > 0 ? Math.ceil(fixedCosts / netProfit) : 0;
      return {
        adSpend,
        breakEvenUnits,
        breakEvenRevenue: breakEvenUnits * sellPrice,
      };
    });
    ...
    <div className="p-6 space-y-6 max-w-5xl mx-auto">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Enter your costs and target margin to find the ideal sell price and
          profit projections.
        </p>
      </div>
          ...
          {results ? (
            <>
              ...
              <Card>
                <CardHeader className="pb-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    How many units you need to sell at each ad spend level to
                    cover your costs and turn a profit.
                  </p>
                </CardHeader>
                <CardContent>
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody className="divide-y divide-border">
                        {results.breakEvenRows.map((row) => {
                          ...
                          const roas =
                            row.adSpend > 0
                              ? results.sellPrice / row.adSpend
                              : null;
                          const marginAfterAds =
                            (net / results.sellPrice) * 100;
                          const isNegative = net <= 0;
                          return (
                            <tr
                              key={row.adSpend}
                              className={isNegative ? "opacity-40" : ""}
                            >
                              <td className="py-2 pr-4 font-medium">
                                {fmt(row.adSpend)}
                              </td>
                              <td
                                className={`py-2 pr-4 text-right font-semibold ${isNegative ? "text-red-400" : "text-green-400"}`}
                              >
                                {fmt(net)}
                              </td>
                              ...
                              <td
                                className={`py-2 text-right font-semibold ${isNegative ? "text-red-400" : ""}`}
                              >
                                {isNegative
                                  ? "Unprofitable"
                                  : pct(marginAfterAds)}
              ...
              <Card>
                <CardHeader className="pb-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Estimated revenue and profit at different order volumes
                    (excluding ad spend).
                  </p>
                </CardHeader>
                <CardContent>
            ...
            <Card className="flex items-center justify-center h-64">
              <p className="text-muted-foreground text-sm">
                Enter a valid cost and margin to see results.
              </p>
            </Card>
          )}
        </div>
