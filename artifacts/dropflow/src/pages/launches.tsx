...
export default function LaunchesPage() {
  ...
  const remove = useMutation({
    mutationFn: (id: number) =>
      fetch(`${BASE}/api/launches/${id}`, { method: "DELETE" }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["launches"] });
  ...
  function toggleStep(launch: Launch, stepKey: string) {
    ...
    update.mutate({
      id: launch.id,
      body: { steps: newSteps, status: newStatus },
    });
  }
  ...
  const stats = {
    ...
    active: launches.filter(
      (l) => l.status === "active" || l.status === "scaling",
    ).length,
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Rocket className="w-6 h-6 text-primary" /> Launch Planner
          </h1>
          <p className="text-muted-foreground text-sm mt-1">
            Step-by-step product launch workflow — source to scale
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 text-muted-foreground">
          ...
          <p className="text-sm">
            No launches yet — create your first product launch plan
          </p>
        </div>
      ) : (
        <div className="space-y-3">
          {launches.map((launch) => {
            const doneSteps = launch.steps.filter(
              (s) => s.status === "done",
            ).length;
            ...
            return (
              <Card
                key={launch.id}
                className={launch.status === "killed" ? "opacity-60" : ""}
              >
                <CardContent className="p-0">
                  <div
                    className="flex items-center gap-4 p-4 cursor-pointer"
                    onClick={() => setExpanded(isOpen ? null : launch.id)}
                  >
                    {isOpen ? (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge
                          variant="outline"
                          className={STATUS_COLORS[launch.status] ?? ""}
                        >
                          {launch.status}
                        </Badge>
                      </div>
                    ...
                    >
                      ...
                      <button
                        onClick={() => remove.mutate(launch.id)}
                    ...
                    <div className="border-t border-border px-4 pb-4 pt-3 space-y-2">
                      {launch.steps.map((step) => (
                        ...
                        >
                          ...
                          <span
                            className={`text-sm ${step.status === "done" ? "line-through text-muted-foreground" : "text-foreground"}`}
                          >
                            {step.title}
                          </span>
                        </div>
