...
const AI_SUGGESTIONS = [
  ...
  "champions from last 30 days",
  "new customers with high potential",
  "VIPs in North America",
  "inactive customers over 90 days",
];
...
export default function CustomerFilters({
  ...
  setSelectedSpend: (v: number) => void;
  selectedEngagement: string;
  setSelectedEngagement: (v: string) => void;
  sortBy: string;
  ...
  const activeFilters = [
    ...
    selectedSpend > 0 && {
      key: "spend",
      label: SPEND_RANGES[selectedSpend]?.label ?? "",
    },
    selectedEngagement !== "All" && {
      key: "engagement",
      label: `${selectedEngagement} Engagement`,
    },
        ...
        <div ref={searchRef} className="relative flex-1">
          <div className="relative">
            ...
            <input
              ...
              placeholder='Search customers... (try "high-value repeat buyers")'
      ...
      <div className="flex flex-wrap items-center gap-2">
          ...
          <motion.button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
              selectedEngagement === level
                ? "bg-gradient-to-r from-emerald-500 to-cyan-500 text-white shadow-[0_0_12px_rgba(16,185,129,0.3)]"
                : "bg-white/[0.04] text-muted-foreground border border-white/[0.06] hover:border-white/[0.12] hover:text-foreground"
            }`}
          >
            {level}
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <select
            ...
            className="px-2.5 py-1.5 rounded-lg bg-white/[0.04] border border-white/[0.08]
              text-xs text-foreground focus:outline-none focus:border-blue-500/40"
          >
            {SPEND_RANGES.map((range, idx) => (
              <option key={range.label} value={idx} className="bg-card">
                {range.label}
              </option>
            ))}
          </select>
          ...
          >
            ...
            {activeFilters.map((f) => (
              ...
              >
                ...
                <button
                  onClick={() => {
                    if (f.key === "region") setSelectedRegion("All");
                    if (f.key === "spend") setSelectedSpend(0);
                    if (f.key === "engagement") setSelectedEngagement("All");
                  }}
