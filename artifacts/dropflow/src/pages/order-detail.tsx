...
export default function OrderDetail() {
  ...
  const { data: timeline = [] } = useQuery<TimelineEvent[]>({
    ...
    queryFn: () =>
      fetch(`${API}/orders/${orderId}/timeline`).then((r) => r.json()),
    enabled: !!orderId,
  });
  ...
  const margin =
    order.sellPrice && order.costPrice
      ? ((Number(order.sellPrice) - Number(order.costPrice)) /
    ...
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <Badge
            className={`text-sm border px-3 py-1 ${STATUS_STYLES[order.status] || STATUS_STYLES.pending}`}
          >
            {order.status}
          </Badge>
          ...
          <CardContent className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="font-semibold">
                {fmt(order.sellPrice, settings.currency)}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Cost Price</span>
              <span>{fmt(order.costPrice, settings.currency)}</span>
            </div>
            <div className="flex justify-between border-t border-border pt-2">
              <span className="text-muted-foreground">Profit</span>
              <span
                className={`font-bold ${Number(order.profit) > 0 ? "text-green-400" : "text-red-400"}`}
              >
                {fmt(order.profit, settings.currency)}
              </span>
            </div>
            {margin != null && (
              <div className="flex justify-between">
                ...
                <span
                  className={`font-medium ${margin >= 30 ? "text-green-400" : margin >= 15 ? "text-yellow-400" : "text-red-400"}`}
                >
                  {margin.toFixed(1)}%
                </span>
              </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <Badge
                className={`text-xs border ${STATUS_STYLES[order.status]}`}
              >
                {order.status}
              </Badge>
            </div>
            ...
            <div className="relative">
              ...
              <div className="space-y-4">
                {timeline.map((event, i) => {
                  const Icon =
                    EVENT_ICONS[event.toStatus || event.event] || AlertCircle;
                  return (
          ...
          <tfoot>
            <tr className="border-t">
              ...
              <td className="pt-3 text-right font-bold">
                {fmt(
                  Number(order.sellPrice) * Number(order.quantity),
                  settings.currency,
                )}
              </td>
            </tr>
