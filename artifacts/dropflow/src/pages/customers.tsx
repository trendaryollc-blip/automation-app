...
const SEGMENT_CONFIG: Record<
  string,
  { color: string; icon: any; desc: string }
> = {
  Champions: {
...
export default function CustomersPage() {
  ...
  const customers = useMemo(() => {
    ...
    return Object.values(map).map((c) => ({
      ...
      deliveredCount: c.orders.filter((o: any) => o.status === "delivered")
        .length,
      cancelledCount: c.orders.filter((o: any) => o.status === "cancelled")
        .length,
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Customer intelligence — LTV, purchase history & RFM segmentation
          </p>
        </div>
        <div className="flex bg-muted rounded-lg p-1 gap-1">
          <button
            onClick={() => setView("list")}
            className={`px-3 py-1.5 rounded text-xs font-medium transition-colors ${view === "list" ? "bg-card shadow text-foreground" : "text-muted-foreground hover:text-foreground"}`}
          >
            Customer List
          </button>
          <button
            onClick={() => setView("rfm")}
            className={`px-3 py-1.5 rounded text-xs font-medium transition-colors ${view === "rfm" ? "bg-card shadow text-foreground" : "text-muted-foreground hover:text-foreground"}`}
          >
            RFM Segments
          </button>
        </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold mt-0.5">{customers.length}</p>
              </div>
                        ...
                        >
                          ...
                          {[c.r, c.f, c.m].map((score, si) => (
                            <td key={si} className="px-3 py-3 text-center">
                              <span
                                className={`w-6 h-6 inline-flex items-center justify-center rounded font-bold text-xs ${score >= 4 ? "bg-emerald-500/20 text-emerald-400" : score >= 3 ? "bg-blue-500/20 text-blue-400" : score >= 2 ? "bg-yellow-500/20 text-yellow-400" : "bg-red-500/20 text-red-400"}`}
                              >
                                {score}
                              </span>
                            </td>
