...
test("Dashboard renders key statistics and stock alert banner", async () => {
  render(<Dashboard />);

  expect(await screen.findByText("War Room")).toBeTruthy();
  expect(screen.getByText("Operational")).toBeTruthy();
  expect(screen.getByText(/running low on stock/)).toBeTruthy();
  expect(screen.getAllByText("Total Revenue")[0]).toBeTruthy();
  expect(screen.getAllByText("Net Profit")[0]).toBeTruthy();
});

test("Dashboard switches period buttons", async () => {
  render(<Dashboard />);
