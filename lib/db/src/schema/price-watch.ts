...
export const priceSnapshotsTable = pgTable("price_snapshots", {
  id: serial("id").primaryKey(),
  watchId: integer("watch_id").notNull().references(() => priceWatchTable.id, { onDelete: "cascade" }),
  price: numeric("price", { precision: 10, scale: 2 }).notNull(),
  note: text("note"),
  recordedAt: timestamp("recorded_at", { withTimezone: true }).notNull().defaultNow(),
});

export const insertPriceWatchSchema = createInsertSchema(priceWatchTable).omit({ id: true, createdAt: true, updatedAt: true });
export const insertPriceSnapshotSchema = createInsertSchema(priceSnapshotsTable).omit({ id: true, recordedAt: true });
